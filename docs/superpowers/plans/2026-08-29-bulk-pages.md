...
| The TMDB client's token bucket is **in-process**, default 30/s | ⚠️ **This does NOT bound concurrency, and the plan originally claimed it did.** `buildTmdbDeps()` builds a **fresh** bucket per invocation, so the ceiling is 30N/s for N concurrent requests and no request can see the others. A whole-branch review measured **8 concurrent chunks (40 names) accepted in 2.40s, nothing throttled** — ~17 names/s, ~60,000/hour from one session. The chunk cap bounds one request, not a person. See the follow-up note. |
...
- **A session rate limiter.** `/api/ui/corpus` caps a chunk at 5, but the in-process TMDB bucket **cannot** pace a concurrent caller — `buildTmdbDeps()` builds a fresh bucket per invocation, so eight parallel chunks run unthrottled. Measured by a whole-branch review: ~17 names/s, ~60,000 names/hour from a single session, against a sweeper that drains 360 jobs/hour, so a cold bulk run enqueues faster than it drains and this page's "run again in a minute or two" becomes false.
  The marginal exposure is about **5×**, not unbounded, because `/api/ui/lookup` was already unthrottled at one lookup per request. **Scope the fix as a session rate limit on `/api/ui/*` as a family, keyed on `user.id` inside `sessionGate`** — not as a patch to the corpus route. It needs a per-user window table, which is a schema change, which is why it is deferred rather than squeezed into the last task of the last plan.
